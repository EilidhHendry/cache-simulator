...
class Cache(object):
    ...
    def increase_lru(self, address):
        tag, index, offset = self.decode(address)
        # Decrease the LRU value for all other blocks in the cache
        for sets in self.cache.keys():
            for block in self.cache[sets]:
                if (self.cache[sets][block] > 0) & (sets != index) & (block != tag):
                    self.cache[sets][block] -= 1
        # Then increase the LRU value of the current block by 1
        self.cache[index][tag] += 1
    ...
    def read_write(self, op, address):
        if self.is_cache_hit(address):
            ...
            if len(self.cache) < (self.n_blocks / self.n_ways):
                self.store(address)
            # Else
            if op == 'R':
                self.r_misses += 1
...
def run():
    simulate_all(mintest, 'testout.json')
